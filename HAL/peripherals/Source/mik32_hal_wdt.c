...
/**  
 * @brief Расчет начального значения отсчета таймера соответствующее времени до перезагрузки
 * @ref WDT_InitTypeDef::ReloadMs "WDT_HandleTypeDef.init.ReloadMs".
 * @param hwdt указатель на структуру WDT_HandleTypeDef, которая содержит
 *                  информацию о конфигурации для модуля WDT.
 * @return Структура с рассчитанным начальным значением
 * @ref WDT_ClockTypeDef.tick и код подобранного делителя частоты
 * @ref WDT_ClockTypeDef.divIndex.
